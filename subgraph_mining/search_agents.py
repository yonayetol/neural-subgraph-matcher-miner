...
class SearchAgent:
    """Base class for search strategies to identify frequent subgraphs in embedding space."""
...
class MCTSSearchAgent(SearchAgent):
    ...
    def has_min_reachable_nodes(self, graph, start_node, n):
        """Optimized reachability check using sparse BFS."""
        visited = set()
        queue = [start_node]
        while queue and len(visited) < n:
            ...
            if node not in visited:
                visited.add(node)
                queue.extend([nbr for nbr in graph.neighbors(node) if nbr not in visited])
    ...
    def step(self):
        ...
        for simulation_n in tqdm(range(self.n_trials // (self.max_pattern_size + 1 - self.min_pattern_size))):
            # Pick seed node
            ...
            for cand_graph_idx, cand_start_node in self.visited_seed_nodes:
                ...
                q_score = (sum(self.cum_action_values[state].values()) / (my_visit_counts or 1))
                uct_score = self.c_uct * np.sqrt(np.log(simulation_n or 1) / (my_visit_counts or 1))
            ...
            while frontier and len(neigh) < self.max_size:
                ...
                for cand_node, cand_emb in zip(frontier, cand_embs):
                    ...
                    q_score = (sum(self.cum_action_values[next_state].values()) / (my_visit_counts or 1))
                    uct_score = self.c_uct * np.sqrt(np.log(parent_visit_counts or 1) / (my_visit_counts or 1))
                    node_score = q_score + uct_score
                ...
                frontier = list(((set(frontier) | set(graph.neighbors(best_node))) - visited) - set([best_node]))
...
class GreedySearchAgent(SearchAgent):
    ...
    def init_search(self):
        ...
        # Initialize beams with smaller batches to reduce memory usage
        self.beam_sets = []
        for trial in range(self.n_trials):
            ...
            frontier = list(set(graph.neighbors(start_node)) - set(neigh))
            visited = set([start_node])
            self.beam_sets.append([(0, neigh, frontier, visited, graph_idx)])
    ...
    def step(self):
        ...
        for beam_set in tqdm(self.beam_sets):
            ...
            for _, neigh, frontier, visited, graph_idx in beam_set:
                ...
                # Process candidates in smaller batches to reduce memory usage
                cand_neighs, anchors = [], []
                for cand_node in frontier:
                    cand_neigh = graph.subgraph(neigh + [cand_node])
                    cand_neighs.append(cand_neigh)
                ...
                for cand_node, cand_emb in zip(frontier, cand_embs):
                    ...
                    new_frontier = list(((set(frontier) | set(graph.neighbors(cand_node))) - visited) - set([cand_node]))
                    new_beams.append((
                        score, neigh + [cand_node],
                        new_frontier, visited | set([cand_node]), graph_idx))

            # Keep only the top `n_beams` candidates to reduce memory usage
            new_beams = sorted(new_beams, key=lambda x: x[0])[:self.n_beams]
            for score, neigh, frontier, visited, graph_idx in new_beams[:1]:
                ...
                if self.rank_method in ["counts", "hybrid"]:
                    self.counts[len(neigh_g)][utils.wl_hash(neigh_g, node_anchored=self.node_anchored)].append(neigh_g)

                if self.analyze and len(neigh) >= 3:
                    emb = self.model.emb_model(utils.batch_nx_graphs(
                        [neigh_g], anchors=[neigh[0]] if self.node_anchored else None)).squeeze(0)
    ...
    def finish_search(self):
        ...
        for pattern_size in range(self.min_pattern_size, self.max_pattern_size + 1):
            if self.rank_method == "hybrid":
                cur_rank_method = "margin" if len(max(self.counts[pattern_size].values(), key=len)) < 3 else "counts"
            else:
            ...
            if cur_rank_method == "margin":
                ...
                for _, neighs in sorted(self.counts[pattern_size].items(),
